...
function changeTheme($theme, $style, $themeStylesheet, $init) {
    ...
    if ($theme === 'light') {
        ...
        $themeStylesheet.href = $baseUrl + 'assets/css/modern/theme-' + $theme + stylePath + rtlString + '.min.css';
    } else if ($theme === 'dark') {
        $logo.attr('src', $mediaUrl + 'assets/images/logo-white.png');
        $body.removeClass('theme-semidark').addClass('theme-dark');
        $themeStylesheet.href = $baseUrl + 'assets/css/modern/theme-' + $theme + rtlString + '.min.css';
...
/**
 * Copy to clipboard
 * @param {string} contentEleId Id of code container element
 * @returns {void}
 */
function copyToClipboard(contentEleId) {
	...
	target.textContent = element.innerHTML;
	...
	if (succeed) {
		notifyUser('The source code has been copied.');
	}
}
